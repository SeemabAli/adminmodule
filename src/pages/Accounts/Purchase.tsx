...
export const Purchase = () => {
    ...
    const routes: Route[] = [
        { id: "1", name: "CHAK BELLI KHAN", companyFreight: 200.00, truckFreight: 10.00 },
        { id: "2", name: "CHOA ROAD", companyFreight: 150.00, truckFreight: 8.00 },
    ];
    ...
    const calculateBags = (tons: number) => {
        ...
        if (ratePerTon !== null) {
            ...
            setTotalPrice(calculatedBags * (calculatedRatePerBag + (freightPerBag || 0)));
        }
    };
    ...
    const handleAddToGrid = () => {
        if (!selectedBrand || !selectedRoute || qtyInTon === null || ratePerTon === null) {
        ...
        const purchaseItem: PurchaseItem = {
            id: editingIndex !== null ? purchaseItems[editingIndex].id : Date.now().toString(),
    ...
    const handleSavePurchaseOrder = () => {
        if (!selectedCompany || !selectedTruck || !selectedDriver || purchaseItems.length === 0) {
            notify.error("Please fill all required fields.");
            return;
        }
        ...
        console.log("Saving purchase order:", {
            ...
            driverName: customDriver || drivers.find(d => d.id === selectedDriver)?.name,
    ...
    const handleDeletePurchaseItems = (index: number) => {
        notify.confirmDelete(() => {
            setPurchaseOrderId(purchaseItems[index].id);
                    ...
                    <div>
                        <label className="block mb-1 font-medium">
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedSaleRoute(e.target.value)}
                                className="select select-bordered w-full"
                            >
            ...
            <div className="bg-base-200 p-4 rounded-lg shadow-md mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block mb-1 font-medium">
                            ...
                            >
                                ...
                                {brands
                                    .filter(brand => !selectedCompany || brand.companyId === selectedCompany)
                                    .map(brand => (
                    ...
                    <div>
                        ...
                        {selectedRoute && (
                            <div className="text-sm text-gray-600 mt-1">
                                Company: {routeFreight.company.toFixed(2)}/
                                Truck: {routeFreight.truck.toFixed(2)}
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div>
                        <label className="block mb-1 font-medium">
                            ...
                            <input
                                ...
                                onChange={(e) => setCommissionPerBag(e.target.value ? parseFloat(e.target.value) : null)}
                                className="input input-bordered w-full"
                            />
                        </label>
                    </div>
                    ...
                    <div>
                        <label className="block mb-1 font-medium">
                            ...
                            <input
                                type="number"
                                value={whtTax ?? ""}
                                onChange={(e) => setWhtTax(e.target.value ? parseFloat(e.target.value) : null)}
                                className="input input-bordered w-full"
                            />
                        </label>
                    </div>
                            ...
                            {purchaseItems.map((item, index) => (
                                <tr key={item.id} className="border-b border-base-300">
                                    ...
                                    <td className="p-3">
                                        ...
                                        <button 
                                            onClick={() => handleDeletePurchaseItems(index)} 
